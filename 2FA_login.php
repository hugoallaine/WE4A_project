<?php
require_once(dirname(__FILE__)."/php_tool/2FA_config.php");

if(!isset($_SESSION['email']) || !isset($_SESSION['isTfaEnabled'])){
    header("Location: ".dirname(__FILE__)."/accounts/profil.php");
}

use RobThree\Auth\TwoFactorAuth;

if(isset($_POST['tfa'])){
    $req = $db->prepare("SELECT authKey FROM tfa INNER JOIN on id_user = users(id) WHERE users(email) = ?");
